...
$(document).ready(function () {
    _pendingInterval = setInterval(listPending, 30000);
    setTimeout(function () {
        $('input.has-target').each(function () {
            var id = $(this).attr('id');
            $("#" + id).change(function () {
                readURL(this);
            });
...
function isValid(selector) {
    ...
    $(selector + ' .required').each(function () {
        if (!this || !this.value || this.value.trim() === '' || this.value.trim() == '0.00') {
            valid = false;
            $(this).addClass('error');
            $('<span class="error-span" >' + _requiredTxt + '</span>').insertAfter(this);
            $(this).focus(function () {
...
function sendPostRequest(redirectUrl, formId, successFun) {
    /// <summary>Make an ajax request</summary>
    ...
    if (isValid(selector)) {
        var action = _baseUrl + $(selector).attr('action');
